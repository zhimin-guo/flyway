 ...
 */
public class DBMSSchema extends Schema<DBMSDatabase, DBMSTable> {
    private static final Log LOG = LogFactory.getLog(DBMSSchema.class);
     ...
     */
    DBMSSchema(JdbcTemplate jdbcTemplate, DBMSDatabase database, String name) {
    ...
    @Override
    protected DBMSTable[] doAllTables() throws SQLException {
        List<String> tableNames = ObjectType.TABLE.getObjectNames(jdbcTemplate, database, this);

        DBMSTable[] tables = new DBMSTable[tableNames.size()];
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new DBMSTable(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }
    ...
    public Table getTable(String tableName) {
        return new DBMSTable(jdbcTemplate, database, this, tableName);
    }
    ...
    public enum ObjectType {
        ...
        TABLE("TABLE", "CASCADE CONSTRAINTS PURGE") {
            @Override
            public List<String> getObjectNames(JdbcTemplate jdbcTemplate, DBMSDatabase database, DBMSSchema schema) throws SQLException {
        ...
        QUEUE_TABLE("QUEUE TABLE") {
            ...
            @Override
            public String generateDropStatement(JdbcTemplate jdbcTemplate, DBMSDatabase database, DBMSSchema schema, String objectName) {
        ...
        MINING_MODEL("MINING MODEL") {
            ...
            @Override
            public String generateDropStatement(JdbcTemplate jdbcTemplate, DBMSDatabase database, DBMSSchema schema, String objectName) {
                return "BEGIN DBMS_DATA_MINING.DROP_MODEL('" + database.quote(schema.getName(), objectName) + "'); END;";
            }
        },
        ...
        DATABASE_LINK("DATABASE LINK") {
            @Override
            public void dropObjects(JdbcTemplate jdbcTemplate, DBMSDatabase database, DBMSSchema schema) {
         ...
         */
        public void dropObjects(JdbcTemplate jdbcTemplate, DBMSDatabase database, DBMSSchema schema) throws SQLException {
         ...
         */
        public static Set<String> getObjectTypeNames(JdbcTemplate jdbcTemplate, DBMSDatabase database, DBMSSchema schema) throws SQLException {
           //TODO 查询数据库下 数据类型
            String query = "SELECT DISTINCT OBJECT_TYPE FROM " + database.dbaOrAll("OBJECTS") + " WHERE OWNER = ? " +
         ...
         * @throws SQLException if retrieving of object types failed.
         */
        public static boolean supportedTypesExist(JdbcTemplate jdbcTemplate, DBMSDatabase database, DBMSSchema schema) throws SQLException {
