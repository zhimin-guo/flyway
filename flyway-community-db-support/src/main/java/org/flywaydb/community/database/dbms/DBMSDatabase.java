...
public class DBMSDatabase extends Database<DBMSConnection> {
    private static final String DBMS_NET_TNS_ADMIN = "dbms.net.tns_admin";

    /**
     * If the TNS_ADMIN environment variable is set, enable tnsnames.ora support for the DBMS JDBC driver.
    ...
    public static void enableTnsnamesOraSupport() {
        ...
        String tnsAdminSysProp = System.getProperty(DBMS_NET_TNS_ADMIN);
        if (StringUtils.hasLength(tnsAdminEnvVar) && tnsAdminSysProp == null) {
            System.setProperty(DBMS_NET_TNS_ADMIN, tnsAdminEnvVar);
        }
    }

    public DBMSDatabase(Configuration configuration, JdbcConnectionFactory jdbcConnectionFactory, StatementInterceptor statementInterceptor) {
        super(configuration, jdbcConnectionFactory, statementInterceptor);
    }

    @Override
    protected DBMSConnection doGetConnection(Connection connection) {
        return new DBMSConnection(this, connection);
    }
